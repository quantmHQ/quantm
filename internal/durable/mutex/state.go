...
const (
	MutexStatusReady     MutexStatus = "mutex__ready"
...
// ready transitions to acquiring state and logs.
func (s *MutexState) ready(ctx workflow.Context) {
	s.to_ready(ctx)
	s.logger.debug(s.Handler.WorkflowExecutionID(), "main", "waiting for lock request ...")
}

// to_locked transitions the state to Locked.
func (s *MutexState) to_locked(ctx workflow.Context) {
	s.Status = MutexStatusLocked
	s.logger.debug(s.Handler.WorkflowExecutionID(), "transition", "to locked")
}
...
func (s *MutexState) to_releasing(ctx workflow.Context) {
	...
	s.logger.debug(s.Handler.WorkflowExecutionID(), "transition", "to releasing")
}
...
func (s *MutexState) to_released(ctx workflow.Context) {
	...
	s.logger.debug(s.Handler.WorkflowExecutionID(), "transition", "to released")
}

// to_timeout transitions the state to Timeout.
func (s *MutexState) to_timeout(ctx workflow.Context) {
	s.Status = MutexStatusTimeout
	s.logger.debug(s.Handler.WorkflowExecutionID(), "transition", "to timeout")
}

// to_ready transitions the state to Acquiring.
func (s *MutexState) to_ready(ctx workflow.Context) {
	s.Status = MutexStatusReady
	s.logger.debug(s.Handler.WorkflowExecutionID(), "transition", "ready again ...")
}

// stop_persisting sets the persist flag to false.
func (s *MutexState) stop_persisting(ctx workflow.Context) {
	s.Persist = false
	s.logger.debug(s.Handler.WorkflowExecutionID(), "persist", "stopped")
}
...
// stop returns a callback for handling the idle timeout.
func (s *MutexState) stop(ctx workflow.Context) func(workflow.ReceiveChannel, bool) {
...
// expire returns a callback for handling the lease expiration.
func (s *MutexState) expire(ctx workflow.Context) func(workflow.Future) {
...
func (s *MutexState) released(ctx workflow.Context) {
	...
	s.logger.info(s.Handler.WorkflowExecutionID(), "main", "lock released")
	s.to_released(ctx)
}
